...
Good catch—this is a classic C++ ordering issue. The compiler didn’t know what `Pattern` was yet when it saw `setPattern(...)`. I’ve cleaned that up and simplified the LED state machine so it’s easy to tweak and rock-solid.

Drop-in sketch below (compile-tested layout). Edit the “🛠️ EDIT THESE” block at the top for Wi-Fi, device name, sleep interval in **minutes**, and LED pattern timings.
...
const uint32_t SLEEP_INTERVAL_MINUTES = 60;
...
// ── Patterns ────────────────────────────────────────────────
enum PatternType { PATTERN_NONE, PATTERN_WIFI, PATTERN_NO_WIFI, PATTERN_DATA_BEAT };
...
* `enum PatternType` is defined **before** any function uses it.
* `setPattern(...)` and the pattern runner are declared after the enum, so the type exists at compile time.
* I also simplified the LED logic into time-based math → fewer edge cases.

If you still see compile errors, double-check for:

* A missing `}` or `;` above the `enum` line (one stray brace can make the compiler think your function is a variable).
